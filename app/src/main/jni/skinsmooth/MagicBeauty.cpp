...
void MagicBeauty::initMagicBeauty(JniBitmap* jniBitmap){
	LOGE("MagicBeauty::initMagicBeauty");
...
void MagicBeauty::unInitMagicBeauty(){
    LOGE("MagicBeauty::unInitMagicBeauty");
	if(instance != NULL)
		delete instance;
	instance = NULL;
}
...
void MagicBeauty::_startBeauty(float smoothlevel, float whitenlevel){
	LOGE("MagicBeauty::smoothlevel=%f---whitenlevel=%f",smoothlevel,whitenlevel);
...
void MagicBeauty::_startWhiteSkin(float whitenlevel){
    LOGE("MagicBeauty::startWhiteSkin");
...
void MagicBeauty::_startSkinSmooth(float smoothlevel){
    LOGE("MagicBeauty::startSkinSmooth");
...
void MagicBeauty::initSkinMatrix(){
	LOGE("MagicBeauty::initSkinMatrix");
	if(mSkinMatrix == NULL)
...
//初始化积分，首先是建立两个积分图，然后即可根据积分图计算公式中的m值和v值。
void MagicBeauty::initIntegral(){
	LOGE("MagicBeauty::initIntegral");
	if(mIntegralMatrix == NULL)
    ...
    delete[] columnSum;
    delete[] columnSumSqr;
    LOGE("MagicBeauty::initIntegral~end");
}
